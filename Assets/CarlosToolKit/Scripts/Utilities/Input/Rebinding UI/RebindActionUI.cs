...
{
    #region RebindActionUI Original
    /// <summary>
    ...
    private bool SwapResetBindings(InputAction action, int bindingIndex)
    {
        ...
        for (int i = 0; i < action.actionMap.bindings.Count; ++i)
        {
            ...
            {
                string warning = $"Duplicate binding found for reset to default: '{newBinding.ToDisplayString()}'";

                DisplayCaption(warning);

                Debug.Log($"Duplicate binding found for reset to default: {newBinding.effectivePath}");
                

                if (string.IsNullOrEmpty(newBinding.overridePath) || string.IsNullOrEmpty(binding.path))
                {
                    Debug.Log($"Invalid binding path during reset. NewBinding overridePath: {newBinding.overridePath}, Binding path: {binding.path}");

                    DisplayCaption($"Invalid binding path. NewBinding overridePath: {newBinding.overridePath}, Binding path: {binding.path}");
    ...
    private bool CheckDuplicateBinding(InputAction action, int bindingIndex, bool allCompositeParts = false)
    {
        ...
        string warning = $"Duplicate binding found '{newBinding.ToDisplayString()}'";
    ...
    [SerializeField]
    private bool excludeMouseButtons = false;
    ...
    private void DisplayCaption(string captionMessage)
    {
        string captionTextFormat = $"{captionMessage}";
